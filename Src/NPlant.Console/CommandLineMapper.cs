        ...
        public static void Map<T>(T instance, string[] args)
        {
            ...
            foreach (var arg in args)
            {
                ...
                if (!argName.StartsWith("--"))
                    throw new ConsoleUsageException(
                        $"Arguments are expected to be in the --foo or --foo:value or --foo:\"value\" format.  Argument '{argName}' could not be parsed.");
            ...
            foreach (var property in properties)
            {
                if (property.HasAttribute<RequiredArgumentAttribute>())
                    throw new ConsoleUsageException(
                        $"Expected argument '--{property.Name.ToLower()}', but was not received");
            }
        }
    }
}